...
class Server{
    ...
    setupPaths(options) {
        ...
        this.pluginPaths.push(path.resolve(this.storagePath, './node_modules/'));
        this.pluginPaths.push(path.resolve(this.storagePath, './plugins/'));
    }
    ...
    async run() {
        ...
        while(!this.shutdown) {
            ...
            if(this.bridge && this.bridge.connected && !this.syncing) {
                ...
                if(this._api.hasRegisterdPlatforms()) {
    ...
    async updateManager(state) {
        ...
        for(var serial in state){
            const platformSerial = await this._api.getDevicePlatform(serial);
            if(platformSerial!==false) {
                if(this._api.isRegisteredDevice(serial)){
                    ...
                    log.warn(`Server: Found iounit ${state[serial].name} (i=${state[serial].id}, serial=${serial}) but no registered device (${platformSerial})`);
                    log.debug(this._api.registeredDevices);
                }
            }
        } 
    }
            ...
            if(!plugin_path) {
                ...
                if(!pjson) {
                    ...
                    if (pjson.engines.node && !semver.satisfies(process.version, pjson.engines.node)) {
                        log.warn("Plugin " + platform + " requires Node version of " + pjson.engines.node + " which does not satisfy the current Node version of " + process.version + ". You may need to upgrade your installation of Node.");
                    }
