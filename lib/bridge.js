...
class Bridge {
    ...
    async createIOUnit(device) {
        ...
        if (serial && serial !== '' && await this.haveIOUnitSn(serial) === false) {
    ...
    async haveIOUnitSn(sn=``) {
        ...
        log.debug(`Bridge.haveIOUnit(${sn}) `);
    ...
    getBatteryEnum(nr,lang){
        if(nr>3 || nr<1) nr=2;
        if(!this.BATTERY[nr][lang]) {
            return this.BATTERY[nr]['en'];
        }
        return this.BATTERY[nr][lang];
    }
