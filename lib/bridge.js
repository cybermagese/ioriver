...
class Bridge {
    ...
    async run(api=this._api) {
        ...
        for(var i=0; i < registered.length; i++) {
            ...
            if(device && device.hasParentUnit()) {
                    ...
                    for(var j=0; j < inputs.length; j++){
                        if(device.need2UpdateInput(inputs[j].name)) {
                            ...
                            if(!resS.ok || !resS.authorized) {
                                log.warn(`Bridge: update failed for iochannel ${device.getIOChannelIdByName(inputs[j].name)} and input ${inputs[j].name}`);
                                log.warn(resS);
                            }
                        } else {
                ...
                } else if(device.isDevice() && device.hasIOChannels()) {
                            ...
                            if(methods.on && methods.off) {
                                if(methods.dim && channels[k].order === device.getConnectionIObyName("dimmer")){
                                    ...
                                    if((device.getState() === "off" || (device.getState() === "dim" && device.getValue()===0)) && channels[k].value !== 0) {
                                        ...
                                        var resDim = await this.api.setIOChannel(channels[k].id,0);
                                        log.debug(resDim);
                                        ...
                                        var resDim = await this.api.setIOChannel(channels[k].id,device.getValue());
                                        log.debug(resDim);
                                    } else if(device.getState() === "on" && channels[k].value !== 100) {
                                        log.info(`Bridge: Set ${device.getName()} current value to 100%.`);
                                        var resDim = await this.api.setIOChannel(channels[k].id,100);
                                        log.debug(resDim);
                                    }
                                    ...
                                    if(device.getState() === "off" && channels[k].value !== 0) {
                                        ...
                                        var resO = await this.api.setIOChannel(channels[k].id,0);
                                        log.debug(resO);
                                    } else if(device.getState() === "on" && channels[k].value !== 1) {
                                        log.info(`Bridge: Set ${device.getName()} current value to on.`);
                                        var resO = await this.api.setIOChannel(channels[k].id,1);
                                        log.debug(resO);
                                    }
    ...
    async updateDevice(device) {
        ...
        log.debug(`   Update device ${device.getName()}`);
        ...
        if(res.ok && res.authorized && res.data) {
            ...
            for(var i = 0; i < d.length; i++) {
                ...
                if(ConnectionIO < 100) {
                    ...
                    log.debug(`        update ${device.getName()}.${name} value to ${device.getBattery()}`);
    ...
    async createIOUnit(device) {
        ...
        log.info(`Creating IOUNIT for device ${name} with serial nummber ${serial}`);
    ...
    async createSensor(device) {
        ...
        for(var i = 0; i < inputs.length; i++) {
            ...
            var r1 = await this.api.createIOChannel(device.getParentUnitId(), inputs[i].name, this.api.def.IOCHANNEL.INPUT, device.getConnectionIObyName(inputs[i].name),`Value=${inputs[i].value};${(typeof inputs[i].unit === 'undefinded'?``:`Unit="${inputs[i].unit}";`)}`);
            log.debug(r1);
        }
    ...
    async createToggle(parentIOUNIT) {
