...
const BATTERY = {
    1: {sv:"OK", en:"OK"},
    2: {sv:"Okänd status", en:"Unknown status"},
    3: {sv:"Låg nivå", en:"Low level"},
...
class Bridge {
    ...
    async run(api=this._api) {
        ...
        for(var i=0; i < registered.length; i++) {
            ...
            if(device && device.hasParentUnit()) {
                ...
                log.warn(device);
                log.warn(`this.isLoaded(${await this.isLoaded()}) && (device.isSensor(${device.isSensor()}) || device.isDevice(${device.isDevice()})`);
                //create IOUnit
                if(await this.isLoaded() && (device.isSensor() || device.isDevice())) {
    ...
    async createIOUnit(device) {
        ...
        if (serial && serial !== '' && serial < 4294967296 && await this.haveIOUnitSn(serial) === false) {
            ...
            if(serial>=4294967296) {
                log.debug(`Bridge: IOUNIT for device ${name} has invalid serial nummber ${serial} !`);
            } else {
                log.debug(`Bridge: IOUNIT for device ${name} with serial nummber ${serial} exist, skip creating!`);
            }
    ...
    async createBattery(device) {
        ...
        if(r1.ok && r1.authorized && r1.data && r1.data && r1.data.i && r1.data.Code === 0) {
            ...
            var r2 = await this.api.formatIOChannel(index1,this.api.def.FORMAT.NAMED,0,0,0,`{'-1':'${this.getBatteryEnum(1,lang)}'},{'-2':'${this.getBatteryEnum(2,lang)}'},{'-3':'${this.getBatteryEnum(3,lang)}'}`);
            log.debug(r2);
        }
    }
    ...
    async haveIOUnitSn(sn=``) {
        ...
        if(res.ok && res.data && res.data.IOUNIT && res.data.IOUNIT.length > 0) {
            log.warn(`Bridge have IOUnit with Sn ${sn}`);
            log.warn(res);
            return true;
        } else {
            //return true if request failed
            if(res.ok===false || typeof res ==='undefined') {
                log.info(`true 2`);
                return true;
            }
        }
    ...
    getBatteryEnum(nr,lang){
        if(nr>3 || nr<1) nr=2;
        if(!BATTERY[nr][lang]) {
            return BATTERY[nr]['en'];
        }
        return BATTERY[nr][lang];
    }
